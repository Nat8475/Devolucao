...
import { cancelarTransferenciaSchema } from '@/lib/validation';
...
export async function POST(request: NextRequest, { params }: { params: Promise<{ loteId: string }> }) {
  ...
  const parsed = cancelarTransferenciaSchema.safeParse(body);
  if (!parsed.success) {
    return NextResponse.json({ error: parsed.error.issues[0].message }, { status: 400 });
  }

  const supabase = await createClient();
  const { data, error } = await supabase.rpc('fn_cancelar_transferencia', {
    p_lote_id: loteId,
